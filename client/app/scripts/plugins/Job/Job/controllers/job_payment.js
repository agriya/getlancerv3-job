...
angular.module('getlancerApp.Job')
    .controller('JobPaymentsController', ['$rootScope', '$scope', '$window', 'countries', 'states', 'cities', 'flash', '$location', '$filter', '$state', 'paymentGateways', 'UserMeFactory', 'PaymentOrderFactory', '$uibModalStack', 'ConstDiscountType', 'CouponGetStatusFactory', 'ConstPaymentGateways', 'md5', 'JobsEdit', 'JobTypeFactory', 'JobCategoriesFactory', 'JobSkillsFactory', 'JobSalaryTypeFactory', 'Upload', '$timeout', '$uibModal', 'DateFormat', 'SalaryType', 'JobsAppliesFactory', 'flagCategories', '$cookies', function($rootScope, $scope, $window, countries, states, cities, flash, $location, $filter, $state, paymentGateways, UserMeFactory, PaymentOrderFactory, $uibModalStack, ConstDiscountType, CouponGetStatusFactory, ConstPaymentGateways, md5, JobsEdit, JobTypeFactory, JobCategoriesFactory, JobSkillsFactory, JobSalaryTypeFactory, Upload, $timeout, $uibModal, DateFormat, SalaryType, JobsAppliesFactory, flagCategories, $cookies) {
        ...
        function job() {
            JobsEdit.get({
                ...
                $scope.plan_info.price_final = response.data.total_listing_fee;
        ...
        $scope.applyCoupon = function() {
            ...
            params.amount = $scope.job_view.zazpay_revised_amount;
            CouponGetStatusFactory.get(params, function(response) {
                ...
                if (response.error.code === 0) {
                    if ($scope.discountCoupon.data.discount_type_id == ConstDiscountType.Amount) {
                        $scope.plan_info.price_final = parseFloat($scope.job_view.zazpay_revised_amount) - parseFloat(response.data.discount);
                        $scope.show_discount = $filter("currency")(response.data.discount);
                    } else {
                        var discount_amt = ((response.data.discount / 100) * $scope.job_view.zazpay_revised_amount);
                        $scope.plan_info.price_final = $scope.job_view.zazpay_revised_amount - discount_amt;
        ...
        $scope.ClearCoupon = function() {
            $scope.plan_info.price_final = $scope.job_view.zazpay_revised_amount;
